...
`TarsTypeAdapter` 类似于 Pydantic 的 `TypeAdapter`，它允许你对任何 Python 类型（不仅仅是 `Struct` 子类）执行 Tarsio 序列化和反序列化操作。
...
from tarsio import TarsTypeAdapter
...
`TarsTypeAdapter` 支持所有 Tarsio 支持的类型提示，包括：
...
当使用 `TarsTypeAdapter` 处理**基础类型**（如 `int` 或 `str`）时，JCE 协议要求所有数据必须包含 Tag。默认情况下，适配器会将这些基础类型包装在 **Tag 0** 中。
...
如果你需要从非 Tag 0 的位置读取数据，可以指定 `id` 参数：
...
`TarsTypeAdapter` 在初始化时会构建类型特定的序列化/反序列化器。因此，建议**创建一次适配器实例并重复使用**，而不是每次操作都重新创建，以获得最佳性能。

```python title="best_practice.py"
# 推荐做法：模块级变量
user_list_adapter = TarsTypeAdapter(list[User])

def process_data(data: bytes):
    users = user_list_adapter.validate_tars(data)
    ...
```
