...
mod tests {
    ...
    #[test]
    fn test_encode_raw_with_map_writes_map_type_id() {
        Python::attach(|py| {
            let inner = PyDict::new(py);
            inner.set_item("a", 1).unwrap();

            let outer = PyDict::new(py);
            outer.set_item(0u8, inner).unwrap();

            let bytes = encode_raw_dict(&outer, 0).unwrap();
            assert_eq!(bytes[0], 0x08);
        });
    }

    #[test]
    fn test_encode_raw_with_list_writes_list_type_id() {
        Python::attach(|py| {
            let list = PyList::empty(py);
            ...
            let outer = PyDict::new(py);
            outer.set_item(0u8, list).unwrap();
    ...
    #[test]
    fn test_encode_raw_with_nested_struct_writes_struct_begin_end() {
        Python::attach(|py| {
            let inner = PyDict::new(py);
            inner.set_item(1u8, 1).unwrap();
            ...
            assert_eq!(*bytes.last().unwrap(), 0x0B);
        });
    }

    #[test]
    fn test_decode_raw_max_depth_exceeded() {
        Python::attach(|py| {
            let mut writer = TarsWriter::new();
            for _ in 0..(MAX_DEPTH + 1) {
                writer.write_tag(0, TarsType::List);
                writer.write_int(0, 1);
            }
            writer.write_int(0, 1);

            let bytes = writer.into_inner();
            let err = decode_raw(py, &bytes).unwrap_err();
